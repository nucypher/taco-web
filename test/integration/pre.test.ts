...
it('makes retrievalKits correctly', async () => {

  // load some bytes generated by a python RetrievalKit
  const rKitb64 = 'UktpdAABAACSxGIC0+CeUlEPis9LRNbrDzEOBkovxRsoxLlAMlvh3dglW+0DcfvuGqJJeWjJr7gPkIpWV/gfLMmkhvw0Eiw+QQ5QKR4iCDD3Ba42qlUPDoWqiXBUa3b0e8/3TZAHzcFI/JzTUZA='
  const bytes = new Uint8Array(Buffer.from(rKitb64, 'base64'))
  const rkit = RetrievalKit.fromBytes(Buffer.from(rKitb64, 'base64'))
  expect(rkit.toBytes()).toEqual(bytes)
})