import { Capsule, PublicKey, Signature, Signer, VerifiedCapsuleFrag } from 'umbral-pre';

import { Bob } from '../characters/bob';
import { IUrsula } from '../characters/porter';
import { encryptAndSign, keccakDigest } from '../crypto/api';
import { ETH_ADDRESS_BYTE_LENGTH, ETH_HASH_BYTE_LENGTH, HRAC_LENGTH } from '../crypto/constants';
import {
...
export class PublishedTreasureMap {
  ...
  public orient(aliceVerifyingKey: PublicKey, bob: Bob) {
    const decryptedMap = bob.verifyFrom(aliceVerifyingKey, this.messageKit, true);
  ...
  private bytesToDestinations(bytes: Uint8Array): Record<ChecksumAddress, Uint8Array> {
...
export class TreasureMap {
  ...
  public prepareForPublication(
    ...
    const publicSignature = aliceSigner.sign(new Uint8Array([...aliceSignerPk, ...hrac])).toBytes();
    const payload = new Uint8Array([...publicSignature, ...hrac, ...messageKit.toBytes()]);
  ...
  private makeHrac(aliceSignerPk: Uint8Array, bobVerifyingKey: PublicKey, label: string) {
