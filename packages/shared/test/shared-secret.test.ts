...
import { encryptWithSharedSecret, decryptWithSharedSecret } from '../src/encryption/shared-secret';
import { DecryptionError, DecryptionErrorCode, EncryptionError, EncryptionErrorCode } from '../src/encryption/errors';
...
describe('Shared Secret Encryption/Decryption', () => {
  ...
  beforeEach(() => {
    ...
    plaintext = new TextEncoder().encode('This is a test message for encryption and decryption.');
  });
  ...
  describe('encryptWithSharedSecret', () => {
    it('should encrypt plaintext successfully', () => {
      ...
      expect(Buffer.from(encrypted).toString('hex')).not.toEqual(Buffer.from(plaintext).toString('hex'));
    ...
    it('should produce different ciphertexts for the same plaintext due to random nonce', () => {
      ...
      expect(Buffer.from(encrypted1).toString('hex')).not.toEqual(Buffer.from(encrypted2).toString('hex'));
    });
    ...
    it('should throw EncryptionError on encryption failure', () => {
      ...
      expect(() => encryptWithSharedSecret(invalidSecret, plaintext)).toThrow(EncryptionError);
      try {
        encryptWithSharedSecret(invalidSecret, plaintext);
      } catch (error) {
        expect(error).toBeInstanceOf(EncryptionError);
        expect((error as EncryptionError).code).toBe(EncryptionErrorCode.PlaintextTooLarge);
        expect((error as EncryptionError).message).toBe('Plaintext is too large to encrypt');
      }
    });
  });
  ...
  describe('decryptWithSharedSecret', () => {
    it('should decrypt ciphertext back to the original plaintext', () => {
      ...
      expect(Buffer.from(decrypted).toString()).toEqual(Buffer.from(plaintext).toString());
    });
    ...
    it('should handle empty ciphertext appropriately', () => {
      ...
      expect(() => decryptWithSharedSecret(sharedSecret, emptyCiphertext)).toThrow(DecryptionError);
    });
    ...
    it('should throw DecryptionError when ciphertext is too short', () => {
      ...
      expect(() => decryptWithSharedSecret(sharedSecret, shortCiphertext)).toThrow(DecryptionError);
      try {
        decryptWithSharedSecret(sharedSecret, shortCiphertext);
      } catch (error) {
        expect(error).toBeInstanceOf(DecryptionError);
        expect((error as DecryptionError).code).toBe(DecryptionErrorCode.CiphertextTooShort);
        expect((error as DecryptionError).message).toBe('The ciphertext must include the nonce');
      }
    });
    ...
    it('should throw DecryptionError on authentication failure', () => {
      ...
      expect(() => decryptWithSharedSecret(sharedSecret, encrypted)).toThrow(DecryptionError);
      try {
        decryptWithSharedSecret(sharedSecret, encrypted);
      } catch (error) {
        expect(error).toBeInstanceOf(DecryptionError);
        expect((error as DecryptionError).code).toBe(DecryptionErrorCode.AuthenticationFailed);
        // Check that it contains the expected error message
        expect((error as DecryptionError).message).toContain('Decryption of ciphertext failed');
        expect((error as DecryptionError).message).toContain('either someone tampered with the ciphertext or');
        expect((error as DecryptionError).message).toContain('you are using an incorrect decryption key');
      }
    });
    ...
    it('should throw DecryptionError when using wrong shared secret', () => {
      ...
      expect(() => decryptWithSharedSecret(wrongSecret, encrypted)).toThrow(DecryptionError);
      try {
